...
const postService = (function () {
    const getList = async (callback, userNumber) => {
        const response = await fetch(
            "https://jsonplaceholder.typicode.com/posts"
        );
        const posts = await response.json();
        return callback(posts, userNumber);
    };
...
const postLayout = (function () {
    const showList = (posts, userNumber) => {
        return posts.filter(({ userId, ...rest }) => userId === userNumber);
    };
...
const printPost = async () => {
    const result = await postService.getList(postLayout.showList, 4);
    console.log(result);
};
...
const albumLayout = (function () {
    const filterAlbum = (albums) => {
        return albums.filter(({ userId, ...rest }) => userId === 5);
    };

    return { filterAlbum: filterAlbum };
})();

const printAlbum = async () => {
    const albumResult = await albumService.getAlbum(albumLayout.filterAlbum);
